...
public class TimeStringConverter {
  ...
  public long convert(String timeString) {
    ...
    Matcher daysMatcher = PATTERN_DAYS.matcher(input), hoursMatcher = PATTERN_HOURS.matcher(input),
    ...
    try {
      result += TimeUnit.HOURS
          .toMillis(Integer.parseInt(input.substring(hoursMatcher.start(), hoursMatcher.end() - 1)));
    } catch (Exception e) {

    }
    try {
      result += TimeUnit.MINUTES
          .toMillis(Integer.parseInt(input.substring(minutesMatcher.start(), minutesMatcher.end() - 1)));
    } catch (Exception e) {
