  ...
  public void onSignChange(SignChangeEvent event) {
    ...
    if (type == PlotSignType.FOR_RENT) {
      ...
      if (rentCost < minRentCostConverted) {
        ...
        player.sendMessage(
            ChatColor.RED + "The rent-price must be between " + this.mgr.getEconomy().format(minRentCostConverted)
                + " and " + this.mgr.getEconomy().format(maxRentCostConverted) + " per " + rentTimeString + ".");
        player.sendMessage(ChatColor.RED + "In other words, between " + this.mgr.getEconomy().format(minRentCost)
            + " and " + this.mgr.getEconomy().format(maxRentCost) + " per hour.");
        event.setCancelled(true);
        sign.getBlock().breakNaturally();
        return;
      ...
      this.mgr.messages.upForRent(player, region.getOwners().getUniqueIds(), region.getMembers().getUniqueIds(),
      ...
      if (plotWorld.getConfig().isReserveFreeRegionsEnabled()) {
        ...
        if (!homelessUUIDs.isEmpty()) {
          ...
          player.sendMessage(ChatColor.RED
              + "Sorry, you can't sell this region. The following players would become homeless: " + homelessString);
        }
      ...
      if (sellCost < minSellCost) {
        player.sendMessage(ChatColor.RED + "The price is too low.");
        player.sendMessage(ChatColor.RED + "The sell-cost must be between " + this.mgr.getEconomy().format(minSellCost)
            + " and " + this.mgr.getEconomy().format(maxSellCost) + ".");
        event.setCancelled(true);
        sign.getBlock().breakNaturally();
        return;
      ...
      this.mgr.messages.upForSale(player, region.getOwners().getUniqueIds(), region.getMembers().getUniqueIds(),
          ...
          try {
            ...
            if (plot != null) {
              if (plot.getRegion() != null) {
                ...
                event.getPlayer()
                    .sendMessage(ChatColor.RED + "Sorry, region '" + plot.getRegionId() + "' doesn't exist anymore.");
              ...
              event.getPlayer()
                  .sendMessage(ChatColor.RED + "Sorry, this sign is invalid. Couldn't find the plot information.");
              event.getClickedBlock().breakNaturally();
              return;
            }
          } catch (SignException e) {
    ...
    if (event instanceof BlockBreakEvent) {
      ...
      if (!isOwner && !player.hasPermission(Permission.BREAK_ANY_SIGN)) {
        ...
        if (signs == null || signs.isEmpty()) {
          player.sendMessage(ChatColor.GREEN + "You broke the last " + type.name() + " sign of region \""
              + plot.getRegionId() + "\".");
        } else {
          player.sendMessage(ChatColor.GREEN + "You broke a " + type.name() + " sign of region \"" + plot.getRegionId()
              + "\". There are " + signs.size() + " " + type.name() + " signs left.");
        }
      }
    } else {
      // protect the sign
