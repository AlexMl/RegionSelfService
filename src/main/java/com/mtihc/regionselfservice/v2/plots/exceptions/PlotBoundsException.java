...
public class PlotBoundsException extends PlotControlException {
  ...
  public enum Type {
    SELECTION_TOO_SMALL("The selected region is too small..."), SELECTION_TOO_BIG(
        "The selected region is too big!"), SELECTION_TOO_LOW(
            "The bottom-y coordinate is too low."), SELECTION_TOO_HIGH("The top-y coordinate is too high");
  ...
  public PlotBoundsException(Type type, int width, int length, int height, int minimum, int maximum, int minHeight,
      int maxHeight) {
    super(getMessage(type, width, length, height, minimum, maximum, minHeight, maxHeight));
    this.type = type;
  }

  static private String getMessage(Type type, int width, int length, int height, int minimum, int maximum,
      int minHeight, int maxHeight) {
    if (type.equals(Type.SELECTION_TOO_BIG)) {
      return type.getMessage() + " Your selection's size is " + width + "x" + length + "x" + height
          + ". But the maximum width/length/height is " + maximum + "x" + maximum + "x" + maxHeight + ".";
    } else if (type.equals(Type.SELECTION_TOO_SMALL)) {
      return type.getMessage() + " Your selection's size is " + width + "x" + length + "x" + height
          + ". But the minimum is " + minimum + "x" + minimum + "x" + minHeight + ".";
    } else {
      return "Your selection's size is too big or too small. Your selection's size is " + width + "x" + length + "x"
          + height + ".";
    }
