...
public class Messages {
  ...
  public void bought(String regionId, CommandSender buyer, double cost, Set<UUID> owners, Set<UUID> members,
      double share, UUID taxAccountUUID, double tax) {
    ...
    buyer.sendMessage(ChatColor.GREEN + "You bought region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " for "
        + ChatColor.WHITE + format(cost) + ChatColor.GREEN + " from " + ChatColor.WHITE + ownerNames + ChatColor.GREEN
        + ".");
    explainTax(buyer, taxAccountUUID, tax);

    // Region <id> was sold to <buyer>.
    String msg = ChatColor.GREEN + "Region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " was sold to "
        + ChatColor.WHITE + buyer.getName() + ChatColor.GREEN + ".";

    if (owners != null) {
      if (owners.size() > 1) {
        for (UUID ownerUUID : owners) {
          Player owner = Bukkit.getPlayer(ownerUUID);
          if (owner == null || !owner.isOnline() || !owner.hasPermission(permOwner)
              || owner.getName().equalsIgnoreCase(buyer.getName())) {
            continue;
          }
          owner.sendMessage(msg);
          // Sharing <cost> with <owners>
          owner.sendMessage(ChatColor.GREEN + "Sharing " + ChatColor.WHITE + format(cost) + ChatColor.GREEN + " with "
              + ChatColor.WHITE + ownerNames + ChatColor.GREEN + ".");
          // You all received an equal share of <share>
          owner.sendMessage(
              ChatColor.GREEN + "You all received an equal share of " + ChatColor.WHITE + formatShare(cost, owners));
    ...
    if (members != null) {
      for (UUID memberUUID : members) {
        ...
        if (member == null || !member.isOnline() || !member.hasPermission(permMember)
            || member.getName().equalsIgnoreCase(buyer.getName())) {
          continue;
        }
  ...
  private void explainTax(CommandSender sender, UUID taxAccountUUID, double tax) {
    if (tax != 0) {
      sender.sendMessage(ChatColor.WHITE + PlayerUUIDConverter.toPlayerName(taxAccountUUID) + ChatColor.GREEN
          + " received the tax of " + ChatColor.WHITE + format(tax) + ChatColor.GREEN + ".");
    }
  ...
  public void rent_ended(UUID renterUUID, Set<UUID> owners, Set<UUID> members, String regionId, String timeString) {
    ...
    if (renter.isOnline()) {
      renter.getPlayer().sendMessage(ChatColor.RED + "The rent time of " + timeString
          + " has passed. You are no longer a member of region \"" + regionId + "\".");
    }

    String permOwner = Permission.INFORM_OWNER_RENTED;
    String permMember = Permission.INFORM_MEMBER_RENTED;
    String msg = ChatColor.RED + "Player " + renter.getName() + "'s rent time of " + timeString + " has passed. "
        + renter.getName() + " is no longer a member of region \"" + regionId + "\".";

    if (owners != null) {
      for (UUID ownerUUID : owners) {
        Player owner = Bukkit.getPlayer(ownerUUID);
        if (owner == null || !owner.isOnline() || !owner.hasPermission(permOwner)
            || ownerUUID.equals(renter.getUniqueId())) {
          continue;
        }
        owner.sendMessage(msg);
      }
    ...
    if (members != null) {
      for (UUID memberUUID : members) {
        Player member = Bukkit.getPlayer(memberUUID);
        if (member == null || !member.isOnline() || !member.hasPermission(permMember)
            || memberUUID.equals(renter.getUniqueId())) {
          continue;
        }
        member.sendMessage(msg);
      }
  ...
  public void rented(CommandSender renter, Set<UUID> owners, Set<UUID> members, String regionId, double cost,
      String time) {
    ...
    renter.sendMessage(ChatColor.GREEN + "You rented region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " for "
        + ChatColor.WHITE + format(cost) + ChatColor.GREEN + " from " + ChatColor.WHITE + ownerNames + ChatColor.GREEN
        + ", for " + ChatColor.WHITE + time + ChatColor.GREEN + ".");

    // Region <id> was rented out to <buyer> for <time>.
    String msg = ChatColor.GREEN + "Region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " was rented out to "
        + ChatColor.WHITE + renter.getName() + ChatColor.GREEN + " for " + ChatColor.WHITE + time + ChatColor.GREEN
        + ".";
    ...
    if (owners != null) {
      if (owners.size() > 1) {
        for (UUID ownerUUID : owners) {
          Player owner = Bukkit.getPlayer(ownerUUID);
          if (owner == null || !owner.isOnline() || !owner.hasPermission(permOwner)
              || owner.getName().equalsIgnoreCase(renter.getName())) {
            continue;
          }
          owner.sendMessage(msg);
          // Sharing <cost> with <owners>
    ...
    if (members != null) {
      for (UUID memberUUID : members) {
        Player member = Bukkit.getPlayer(memberUUID);
        if (member == null || !member.isOnline() || !member.hasPermission(permMember)
            || member.getName().equalsIgnoreCase(renter.getName())) {
          continue;
        }
        member.sendMessage(msg);
  ...
  public void upForSale(CommandSender seller, Set<UUID> owners, Set<UUID> members, String regionId, double cost) {
    ...
    seller.sendMessage(ChatColor.GREEN + "You put region " + ChatColor.WHITE + regionId + ChatColor.GREEN
        + " up for sale, for " + ChatColor.WHITE + format(cost) + ChatColor.GREEN + ".");
    seller.sendMessage(msg2);

    // Player <seller> put region <id> up for sale, for <cost>.
    String msg = ChatColor.GREEN + "Player " + ChatColor.WHITE + seller.getName() + ChatColor.GREEN + " put region "
        + ChatColor.WHITE + regionId + ChatColor.GREEN + " up for sale, for " + ChatColor.WHITE + format(cost)
        + ChatColor.GREEN + ".";
    ...
    if (owners != null) {
      for (UUID ownerUUID : owners) {
        ...
        if (player == null || !player.isOnline() || !player.hasPermission(permOwner)
            || player.getName().equalsIgnoreCase(seller.getName())) {
          continue;
        }
   ...
   * the region is rented out, the profits are shared amongst "owners".
   */
  public void upForRent(CommandSender letter, Set<UUID> owners, Set<UUID> members, String regionId, double costPerTime,
      String time) {
    ...
    letter.sendMessage(ChatColor.GREEN + "You put region " + ChatColor.WHITE + regionId + ChatColor.GREEN
        + " up for rent, for " + ChatColor.WHITE + format(costPerTime) + ChatColor.GREEN + " per " + ChatColor.WHITE
        + time + ChatColor.GREEN + ".");
    letter.sendMessage(msg2);

    // Player <letter> put region <id> up for rent, for <cost> per <time>.
    String msg = ChatColor.GREEN + "Player " + ChatColor.WHITE + letter.getName() + ChatColor.GREEN + " put region "
        + ChatColor.WHITE + regionId + ChatColor.GREEN + " up for rent, for " + ChatColor.WHITE + format(costPerTime)
        + ChatColor.GREEN + " per " + ChatColor.WHITE + time + ChatColor.GREEN + ".";

    if (owners != null) {
      for (UUID ownerUUID : owners) {
        Player player = Bukkit.getPlayer(ownerUUID);
        if (player == null || !player.isOnline() || !player.hasPermission(permOwner)
            || player.getName().equalsIgnoreCase(letter.getName())) {
          continue;
        }
  ...
  public void removed(CommandSender remover, Set<UUID> owners, Set<UUID> members, String regionId, double refund) {
    ...
    String msg = ChatColor.GREEN + "Player " + ChatColor.WHITE + remover.getName() + ChatColor.GREEN
        + " removed region " + ChatColor.WHITE + regionId + ChatColor.GREEN + ".";
      ...
      if (owners.size() > 1) {
        for (UUID ownerUUID : owners) {
          ...
          if (refund > 0) {
            ...
            player.sendMessage(ChatColor.GREEN + "You all received an equal share of " + ChatColor.WHITE
                + formatShare(refund, owners));
          }
  ...
  public void resized(UUID resizerUUID, Set<UUID> owners, Set<UUID> members, String regionId, double oldWorth,
      double newWorth, int oldWidth, int oldLength, int oldHeight, int newWidth, int newLength, int newHeight) {
    if (oldWidth * oldLength > newWidth * newLength) {
      resized_smaller(resizerUUID, owners, members, regionId, newWorth - oldWorth, oldWidth, oldLength, oldHeight,
          newWidth, newLength, newHeight);
    } else {
      resized_bigger(resizerUUID, owners, members, regionId, newWorth - oldWorth, oldWidth, oldLength, oldHeight,
          newWidth, newLength, newHeight);
    }
   ...
   * CREATE creator Region "id" protected. ...show region info....
   */
  public void resized_bigger(UUID resizerUUID, Set<UUID> owners, Set<UUID> members, String regionId, double cost,
      int oldWidth, int oldLength, int oldHeight, int newWidth, int newLength, int newHeight) {
    ...
    msg += "resized region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " from " + ChatColor.WHITE + oldSize
        + ChatColor.GREEN + " to " + ChatColor.WHITE + newSize + ChatColor.GREEN + ".";
    ...
    if (owners != null) {
      for (UUID ownerUUID : owners) {
        Player player = Bukkit.getPlayer(ownerUUID);
        if (player == null || !player.isOnline() || !player.hasPermission(permOwner)
            || player.getName().equalsIgnoreCase(resizer.getName())) {
          continue;
        }
  ...
  public void resized_smaller(UUID resizerUUID, Set<UUID> owners, Set<UUID> members, String regionId, double refund,
      int oldWidth, int oldLength, int oldHeight, int newWidth, int newLength, int newHeight) {
    ...
    Player resizer = Bukkit.getPlayer(resizerUUID);
    resizer.sendMessage(ChatColor.GREEN + "Resized region " + ChatColor.WHITE + regionId + ChatColor.GREEN + " from "
        + ChatColor.WHITE + oldSize + ChatColor.GREEN + " to " + ChatColor.WHITE + newSize + ChatColor.GREEN + ".");
    ...
    if (owners != null) {
      if (owners.size() > 1) {
        for (UUID ownerUUID : owners) {
          ...
          player.sendMessage(ChatColor.GREEN + "You are owner of that region.");
          if (refund != 0) {
            player.sendMessage(ChatColor.GREEN + "Sharing the refund of " + ChatColor.WHITE + format(Math.abs(refund))
                + ChatColor.GREEN + " with " + ChatColor.WHITE + ownerNames + ChatColor.GREEN + ".");
            player.sendMessage(ChatColor.GREEN + "You all got an equal share of " + ChatColor.WHITE
                + formatShare(Math.abs(refund), owners));
          }
        ...
        if (player != null && player.isOnline() && player.hasPermission(permOwner)) {
          ...
          if (refund != 0) {
            player.sendMessage(
                ChatColor.GREEN + "You received a refund of " + ChatColor.WHITE + format(Math.abs(refund)));
          }
        }
      }
  ...
  private String getResizeMessage(String resizerName, String regionId, String oldSize, String newSize) {
    return ChatColor.GREEN + "Player " + ChatColor.WHITE + resizerName + ChatColor.GREEN + " resized region "
        + ChatColor.WHITE + regionId + ChatColor.GREEN + " from " + ChatColor.WHITE + oldSize + ChatColor.GREEN + " to "
        + ChatColor.WHITE + newSize + ChatColor.GREEN + ".";
  }
