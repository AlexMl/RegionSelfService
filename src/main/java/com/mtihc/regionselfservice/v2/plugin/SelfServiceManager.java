...
public class SelfServiceManager extends PlotManager {
  public SelfServiceManager(JavaPlugin plugin, WorldGuardPlugin worldGuard, IEconomy economy,
      IPlotManagerConfig config) {
    super(plugin, worldGuard, economy, config, new PlotWorldConfigDefault(plugin,
        plugin.getDataFolder() + File.separator + "worlds" + File.separator + "world-config.yml", "world-config.yml"));
  }

  protected PlotWorld createPlotWorld(World world) {

    File worldDir = new File(
        this.plugin.getDataFolder() + File.separator + "worlds" + File.separator + world.getName());
