...
public class EconomyVault implements IEconomy {
	...
	if (!res.transactionSuccess()) {
	    this.logger.log(Level.WARNING, "Failed to deposit " + amount + " to " + accountHolder.getName() + ": " + res.errorMessage);
	}
	...
	if (!res.transactionSuccess()) {
		...
		this.logger.log(Level.WARNING, "Failed to withdraw " + amount + " from " + accountHolder.getName() + ": " + res.errorMessage);
	    }
